...
\begin{document}
	...
	First, the hybrid storage model is adopted: the complete product data is fixed on IPFS through Pinata \cite{Benet2014IPFS}, and only the content identifier (CID), \texttt{keccak256} integrity hash, issuer address and timestamp \cite{Wood2014Ethereum} are recorded on the chain, thus reducing gas costs. Secondly, commercially sensitive data is designed to be kept off-chain; the on-chain hash is used as an integrity anchor to confirm that the off-chain document has not been tampered with, and at the same time, its content will not be exposed to the ledger. Third, all read queries use public RPC endpoints, allowing consumers and regulators to view any passport without holding wallets or paying gas fees.
	...
	EcoPassEU can be modified for deployment on Layer-2 networks such as 
	Polygon to benefit from its EVM compatibility and near-instant finality 
	with significantly lower transaction fees. In the unlikely event that 
	secondary-market transfer becomes mandatory, the registry can be 
	reconstructed around ERC-1155 tokens \cite{Radomski2018ERC1155}.
	
	
	\section{Conclusion}
	EcoPassEU demonstrates that a public blockchain can serve as a 
	practical, tamper-evident foundation for ESPR-compliant Digital 
	Product Passports. By combining Ethereum smart contracts 
	\cite{Antonopoulos2018Mastering} with IPFS-based off-chain 
	storage, the platform achieves independently verifiable 
	sustainability records without imposing wallet requirements on 
	end users. The \texttt{keccak256} hash pairing mechanism 
	\cite{NIST2015SHA3} and Ethereum's Proof-of-Stake consensus 
	\cite{EthereumMerge2022} together address both data integrity 
	and the carbon-footprint concerns that would otherwise undermine 
	a sustainability-focused platform. Batch registration reduces 
	per-unit gas costs by 26\%, making the system economically 
	accessible to SMEs.
	
	The platform's current limitations are clear: testnet-only 
	deployment leaves real-world gas costs unvalidated, reliance on 
	a single Pinata gateway introduces a central point of failure, 
	and the absence of KYC means product identity cannot be 
	guaranteed. Immutability, while essential for tamper evidence, 
	also prevents correction of honest registration errors.
	
	Addressing these gaps --- through mainnet deployment, 
	decentralised identifiers \cite{W3C2025DID,W3C2025VC}, multi-node IPFS storage via 
	Filecoin, and zero-knowledge proofs for selective compliance 
	disclosure --- represents a concrete and achievable roadmap for 
	taking EcoPassEU from proof-of-concept to production.
